...
describe("Follow-up read-only product boundary", () => {
  ...
  it("shows Candidate for Established as an owner decision and links detail only through safe identity", () => {
    ...
    const englishDetail = render("en", React.createElement(CandidateDetailView, { candidate, followUp: entry, initialActiveLayer: "observation" }));
    const polishDetail = render("pl", React.createElement(CandidateDetailView, { candidate, followUp: entry, initialActiveLayer: "observation" }));
