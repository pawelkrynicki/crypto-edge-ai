...
- Supported preview links include `#candidate-results`, `#candidate-detail`, `#token-lookup`, `#external-checks`, `#trusted-preview`, `#feedback-notes`, `#webinar-teaser`, and `#control-center`.
...
## 12E.3 Candidate Detail View

12E.3 adds the second product-facing view: **Candidate Detail** for reviewing one research candidate.

- Candidate Detail is available from navigation and by direct deep link at `#candidate-detail`.
- Candidate Results links to it with `Open candidate detail`.
- The view uses the existing scanner mock/API bridge data and adds no backend, storage, provider call, source activation, dependency, scoring change, `final_label` change, or `WATCHLIST` meaning change.
- It shows token/project name, symbol, chain/network, contract state, research priority, reason on radar, candidate summary, source freshness, source coverage, risk flags, security notes, liquidity / market context, open questions, manual review status, and next review step.
- `WATCHLIST` is presented as manual review only.
- Missing contract, chain, security, liquidity, or freshness data is shown as `manual verification required`, `unknown`, or `not verified`.
- Real token lookup, honeypot links, external verification URL builder, user notes/storage, backend, provider calls, and investment recommendations remain out of scope.
- Next stage: **12E.4 Token / Contract Lookup Shell**.

## 12E.4 Token / Contract Lookup Shell

12E.4 adds a frontend-only **Token / Contract Lookup** shell for local token/contract input classification.

- Token / Contract Lookup is available from navigation and by direct deep link at `#token-lookup`.
- Candidate Detail links to it with `Open token lookup`.
- The view has one input field, quick examples, and local classification for likely symbol, likely project name, likely EVM contract address, likely URL, and unknown format.
- Contract-like input stays `not verified`; chain stays `chain unknown / verify manually`; external checks stay `external check required`.
- Symbol, project, URL, unknown, missing security, missing liquidity, and missing source states stay `contract required`, `manual verification required`, `unknown`, or `not verified`.
- This stage adds no backend, storage, provider call, source activation, URL fetch, scraping, external verification URL builder, dependency, scoring change, `final_label` change, or `WATCHLIST` meaning change.
- Next stage: **12E.5 External Verification Links**.

## 12E.5 External Verification Links

12E.5 adds a frontend-only **External Checks** view for link-only manual verification.

- External Checks is available from navigation and by direct deep link at `#external-checks`.
- Candidate Detail and Token / Contract Lookup link to it with `Open external checks`.
- The view uses a central frontend registry for explorer/manual address check, DEX/liquidity manual check, honeypot/security manual check, and source/context manual check targets.
- External links are static `href` values opened only by user click with `target="_blank"` and `rel="noreferrer noopener"`.
- If a safe prefilled link cannot be built, the view shows manual fallback plus `copy contract` or `copy token input`.
- Missing contract, unknown chain, security, liquidity, source freshness, and WATCHLIST states remain `contract required`, `chain unknown`, `not verified`, `manual verification required`, `security not verified`, `liquidity unknown`, `source freshness unknown`, or manual review only.
- This stage adds no backend, storage, provider call, source activation, external fetch, scraping, dependency, scoring change, `final_label` change, or `WATCHLIST` meaning change.
- Next stage: **12E.6 Manual Verification Fallbacks**.

## 12E.6 Manual Verification Fallbacks

12E.6 adds shared frontend-only **Manual Verification Fallbacks** across the candidate flow.

- Manual fallback panels are used in Candidate Results, Candidate Detail, Token / Contract Lookup, and External Verification Links.
- Shared fallback names are `manual verification required`, `not verified`, `contract required`, `chain unknown`, `security not verified`, `liquidity unknown`, `source freshness unknown`, `external check required`, `manual review only`, and `cannot infer safety`.
- Each fallback includes a short `next review step`.
- Missing data is shown as a `data gap`; it is not treated as a positive status and cannot infer safety.
...
12E.11 polishes the standalone frontend preview so the candidate workflow reads as a coherent product surface rather than a developer mock.

- Candidate Results, Candidate Detail, Token Lookup, External Checks, Manual Verification notices, Product State notices, Research Action Panel, sidebar, header, badges, cards, spacing, and responsive behavior were visually tightened.
- The change is UI polish only. It keeps the existing flow, deep links, data meaning, scanner labels, scoring, `final_label`, and `WATCHLIST` semantics unchanged.
...
- **Candidate Detail View**: Product-facing detail surface at `#candidate-detail` with reason on radar, source freshness, source coverage, risk flags, security notes, open questions, manual review status, and next review step.
...
- **Candidate Detail Panel**: In-depth scanner work area for a selected token, including research context/data coverage, manual checks, and risk reasons.
