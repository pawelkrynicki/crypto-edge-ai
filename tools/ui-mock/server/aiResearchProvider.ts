...
function createOpenAIResearchProvider(options: OpenAIResearchProviderOptions): AIResearchProvider {
  ...
  return {
    ...
    async generate(context, repairErrorCode) {
      ...
      if (!config.apiKey) throw new AIResearchProviderError("MISSING_API_KEY");
      ...
      try {
        ...
        if (!response.ok) throw new AIResearchProviderError("PROVIDER_ERROR");
        const payload: unknown = await response.json();
        const parsed = parseResponsesPayload(payload);
        return { raw_json: parsed.text, model: config.model, token_usage: parsed.usage };
      } catch (error) {
...
export function buildSystemPrompt(locale: "pl" | "en"): string {
  return [
    ...
    "Choose next action types only from allowed_action_types. Recommend research steps only.",
