...
function isRadar(value: unknown): value is LifecycleRadarView { return record(value) && value.schema_version === "lifecycle_radar_view_v1" && isSummary(value.summary) && record(value.actor) && isGroup(value.new_inbox) && record(value.follow_up) && isGroup(value.follow_up.action_due) && isGroup(value.follow_up.candidates_ready) && isGroup(value.follow_up.observed) && record(value.main_radar) && Number.isSafeInteger(value.main_radar.total) && Number.isSafeInteger(value.private_new_total) && Number.isSafeInteger(value.private_follow_up_total) && Number.isSafeInteger(value.private_main_radar_total) && record(value.private_baskets) && isGroup(value.private_baskets.new) && isGroup(value.private_baskets.follow_up) && isGroup(value.private_baskets.main_radar); }
