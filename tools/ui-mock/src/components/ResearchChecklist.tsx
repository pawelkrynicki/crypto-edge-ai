...
function SocialManualEvidenceEditor({ item, candidate, writable, locale, onSaved }: {
  ...
  return <article className="research-social-manual-item" data-pc3d-manual-item={key}>
    ...
    {writable && <ActionButton variant="secondary" onClick={() => setExpanded((current) => !current)}>{expanded ? (pl ? "Zamknij" : "Close") : evidence ? (pl ? "Edytuj wynik" : "Edit result") : (pl ? "Dodaj wynik ręczny" : "Add manual finding")}</ActionButton>}
