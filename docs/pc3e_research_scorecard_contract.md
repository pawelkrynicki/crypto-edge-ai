...
Step 6 exposes a beginner-first `earned / 100` scorecard, explicit Narrative gap, counters, red-flag notice, and non-advisory helper copy. Its label follows the calculated `partial` flag (`Wynik częściowy`/`Partial score` or `Wynik kompletny`/`Complete score`). A compact coverage line derives `resolved / applicable` only from Security, On-chain, Social, and Narrative. When evidence is missing, Step 6 states that the scorecard is incomplete and links only to the incomplete Security, On-chain, and Social research steps through the existing Research Playbook navigation. The compact `W scorecardzie` / `In scorecard` counters derive from the same four scorecard domains; they never borrow the global Step 1 or unique Step 2 readiness counts. When the full Research Playbook has a red flag from any step, Step 6 presents a separate pluralized full-research warning instead of mixing scopes. Its calculation detail is collapsed by default and uses symbols, labels, and translated evidence states only—never IDs, provider internals, or raw JSON.

Step 7 exposes one readiness status, score, the same global checked/red/missing counters, one non-trading next action, and a collapsed meaningful checklist grouped by Steps 1–6. Its Step 2 group contains the derived composite (never `0/0`) and explains that the remaining Deal Breaker controls are counted once in their corresponding Security, On-chain, and Social sections. Its Step 6 entry is a non-counting visual scorecard summary: the current total, three domain rows with missing/red values, and the unresolved Narrative state. It explicitly states that these references summarize Steps 3–5 and are not counted again in the final counters.
