...
W izolowanym lokalnym runtime PC.1 (`?pc1_review=1`, `CRYPTO_EDGE_PC1_REVIEW_MODE=1` i `CRYPTO_EDGE_PC1_REVIEW_ROOT`) harness po 60 sekundach przygotowuje w `REVIEW_ROOT/output/<review-run>/full_output.json` kompletny snapshot review, przepuszcza go przez kanoniczny display validator i dopiero wtedy atomowo zapisuje marker `pc1-review-publication.json`. Następny zwykły odczyt `GET /api/product/version` widzi nowszy review `scanner_run_id` i `scanner_generated_at`, więc UI wykonuje normalny bounded refresh. Błąd przygotowania lub walidacji nie publikuje V2 — V1 pozostaje aktywne. Harness nie uruchamia collectora, providerów, OpenAI, centralnego cycle ani zapisu kanonicznych danych. Lokalny, loopbackowy `POST /api/product/review/publish-next?pc1_review=1` pozostaje wyłącznie pomocniczym przełącznikiem review.
